...
module plu #(parameter [31:0] SEED = 32'd0) (
    ...
    mac plu_mac(.term_spin_j(term_spin_j), .term_spin_k(term_spin_k), .term_valid(term_valid), .fetch_done(fetch_done), .next_color(next_color), .clk(clk), .rst_b(rst_b), .term_type(term_type), .term_weight(term_weight), .sum(sum), .mac_done(mac_done));
